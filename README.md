# Retirement Planner v3.5 PWA

這是一個可維護的退休規劃 PWA 專案，支援本機執行、GitHub Pages 部署與離線使用。

## 功能

- 2026 起始可投資資產：可用拉桿與數值輸入調整。快速情境改以淨資產 1.4 億到 3.0 億、每 0.2 億切換，系統依目前投資資產與淨資產差額換算可投資資產。
...
- 退休決策矩陣：淨資產 1.4 億到 3.0 億，每 0.2 億一跳，並同時顯示換算後可投資資產。
- 圖表固定 Y 軸刻度：資產 0–12 億、年度支出 0–1200 萬、貸款 0–6000 萬、退休時間雙 Y 軸 0–100% / 0–10%。
- Portfolio Engine：新增 CAGR、波動/風險、Sharpe 的成分貢獻分析。
